...
import { $t as translate } from '../hydrate'

export let state: FrameworkGallerySvelteNodeProperties
export let onPatch: <K extends keyof FrameworkGallerySvelteNodeProperties>(
  key: K,
  value: FrameworkGallerySvelteNodeProperties[K],
) => void

$: total = Number(state.price || 0) * Number(state.quantity || 0)
</script>

<section class="overflow-hidden rounded-xl border border-slate-200 bg-white">
  <header class="flex items-center justify-between gap-3 border-b border-slate-200 px-3.5 py-2.5">
    <div>
      <h2 class="m-0 text-xs font-semibold">01 · {translate('panel.configuration')}</h2>
      <p class="mt-0.5 text-[10px] text-slate-500">{translate('panel.savedOnDone')}</p>
    </div>
    <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-[10px] font-medium text-slate-500">
      ● {translate('panel.persisted')}
    </span>
  </header>

  <div class="grid gap-3 p-3.5">
    <label>
      <span class="mb-1.5 block text-[11px] font-medium text-slate-500">{translate('label.name')}</span>
      <input
        id="node-input-name"
        ...
        value={state.name || ''}
        on:input={event => onPatch('name', event.currentTarget.value)}
      />
    </label>
    ...
    <div class="grid grid-cols-2 gap-3">
      <label>
        ...
        <select
          ...
          on:change={event => onPatch('framework', event.currentTarget.value)}
        >
          <option>Vue</option>
          <option>React</option>
      ...
      <label>
        ...
        <select
          ...
          class="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-[13px] text-slate-900 transition outline-none focus:border-indigo-400 focus:ring-3 focus:ring-indigo-100"
          value={state.mode}
          on:change={event => onPatch('mode', event.currentTarget.value)}
        >
    ...
    <label class="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-slate-200 px-3 py-2.5">
      ...
      <input
        ...
        on:change={event => onPatch('enabled', event.currentTarget.checked)}
      />
    </label>
    ...
    <div class="grid grid-cols-[1fr_1fr_auto] items-end gap-3 max-[480px]:grid-cols-2">
      <label>
        ...
        <input
          ...
          min="0"
          value={state.price}
          on:input={event => onPatch('price', Number(event.currentTarget.value || 0))}
        />
      </label>
      ...
      <label>
        ...
        <input
          ...
          on:input={event => onPatch('quantity', Number(event.currentTarget.value || 0))}
        />
      </label>

      <div class="min-w-20 rounded-lg bg-indigo-50 px-3 py-2 text-indigo-700">
        <small class="block text-[9px] font-medium tracking-wide text-indigo-500 uppercase">Total</small>
