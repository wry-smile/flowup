...
RED.nodes.registerType<FrameworkGalleryVueNodeClientNodeProperties>(NODE_NAME, {
  category: 'function',
  color: '#a6bbcf',
  icon: 'vue-node-icon.svg',
  defaults: {
  ...
  oneditprepare() {
    ...
    const target = document.querySelector(
      '#dialog-form [data-flowup-scope="' + NODE_SCOPE + '"].flowup-vue-root',
    )
    if (target) {
      app = createApp(App)
      app.mount(target)
