...
export default function nodeInit(RED: NodeAPI): void {
  function FrameworkGallerySolidNodeNodeConstructor(this: RedNode, config: FrameworkGallerySolidNodeNodeDef): void {
    RED.nodes.createNode(this, config as NodeDef)
    const node = this
    const settings = {
      framework: config.framework || 'Solid',
      mode: config.mode || 'normal',
      enabled: config.enabled !== false,
      price: Number(config.price ?? 12),
      quantity: Number(config.quantity ?? 2),
      items: Array.isArray(config.items) ? [...config.items] : ['Solid', 'Vue', 'Solid'],
    }
  ...
  RED.nodes.registerType<FrameworkGallerySolidNodeNode, FrameworkGallerySolidNodeNodeDef, {}, {}>(
    NODE_NAME,
    FrameworkGallerySolidNodeNodeConstructor,
  )
}
