...
export async function scanFlowupPackages(
  options: ScanFlowupPackagesOptions = {},
): Promise<ScanFlowupPackagesResult> {
  const cwd = resolve(options.cwd ?? process.cwd())
  const monorepo = await isInMonorepo(cwd)
  const rootDir = monorepo ? ((await findWorkspaceRoot(cwd)) ?? cwd) : cwd

  const records: FlowupPackageRecord[] = []
  await walkForFlowupPackages(rootDir, rootDir, records)

  const filtered = filterPackages(records, options.packages).sort((left, right) =>
...
async function walkForFlowupPackages(
  ...
  if (IGNORED_DIR_NAMES.has(basename(currentDir))) return
...
function readFlowupPackageRecord(
  ...
  if (!isFlowupNodeRedField(nodeRed)) return null

  if (!hasNodeRedEntries(nodeRed)) return null
...
function findFlowupConfigFile(packageDir: string): string | null {
  for (const fileName of FLOWUP_CONFIG_FILES) {
    ...
    if (looksLikeFlowupConfig(filePath)) return filePath
  }
...
function filterPackages(
  ...
  const filtered = records.filter(record => {
    ...
    for (const match of matches) matchedFilters.add(match)
    return matches.length > 0
  })

  const missingFilters = normalizedFilters.filter(filter => !matchedFilters.has(filter))
  if (missingFilters.length) {
    throw new Error(`Requested Flowup package(s) not found: ${missingFilters.join(', ')}`)
  }
