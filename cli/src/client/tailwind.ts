  ...
  return function useTailwindcss(): void {
    ...
    const inject = (): void => {
      const shadowRoot = useShadowRoot()
        ?? ((instance?.vnode.el as HTMLElement | null | undefined)?.shadowRoot ?? null)
      if (!shadowRoot)
        return
...
export function getStyleSheet(
  ...
  const resolvedCssText = options.rewriteRootToHost === false
    ? cssText
    : cssText.replace(ROOT_SELECTOR_RE, '$1:host')

  const sheet = new CSSStyleSheet()
  sheet.replaceSync(resolvedCssText)

  if (options.appendPropertyFallbacks !== false)
    appendPropertyFallbacks(sheet, resolvedCssText)
...
function appendPropertyFallbacks(sheet: CSSStyleSheet, cssText: string): void {
  ...
  if (declarations.size > 0) {
    const fallbackText = Array.from(declarations, ([name, value]) => `${name}: ${value}`)
      .join(';')
...
function collectCustomPropertyDeclarations(
  ...
  while (cursor < cssText.length) {
    ...
    if (cssText[valueEnd] === ';') {
      ...
      if (value && !declarations.has(name))
...
function collectPropertyInitialValues(
  cssText: string,
  declarations: Map<string, string>,
): void {
  let cursor = 0
  while (cursor < cssText.length) {
    ...
    for (const declaration of cssText.slice(blockStart + 1, blockEnd).split(';')) {
      ...
      if (separator === -1)
        continue
      if (declaration.slice(0, separator).trim() !== 'initial-value')
        continue
...
function findIdentifierEnd(cssText: string, start: number): number {
  let cursor = start
  while (cursor < cssText.length && isIdentifierCharacter(cssText[cursor]!))
    cursor += 1
  return cursor
}
