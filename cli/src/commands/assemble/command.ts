...
export function registerAssembleCommand(program: Command): void {
  program
    ...
    .option(
      '--output <path>',
      'Output directory, defaults to workspace dist/flowup-assemble or a safe sibling for a single package',
    )
    ...
    .option(
      '--packages <csv>',
      'Only include selected packages by package name, folder name, or relative path',
    )
...
function toAssembleOptions(options: AssembleCommandOptions): AssembleCommandOptions {
  return {
    cwd: hasArgvFlag('--cwd') ? (stripQuotes(options.cwd) as string) : undefined,
    config: hasArgvFlag('--config') ? (stripQuotes(options.config) as string) : undefined,
    output: hasArgvFlag('--output') ? (stripQuotes(options.output) as string) : undefined,
    name: hasArgvFlag('--name') ? (stripQuotes(options.name) as string) : undefined,
    version: hasArgvFlag('--version') ? (stripQuotes(options.version) as string) : undefined,
    description: hasArgvFlag('--description')
      ? (stripQuotes(options.description) as string)
      : undefined,
    author: hasArgvFlag('--author') ? (stripQuotes(options.author) as string) : undefined,
    license: hasArgvFlag('--license') ? (stripQuotes(options.license) as string) : undefined,
    packages: hasArgvFlag('--packages') ? (stripQuotes(options.packages) as string) : undefined,
    clean: hasArgvFlag('--no-clean') ? false : undefined,
    skipBuild: hasArgvFlag('--skip-build') ? true : undefined,
  }
}
