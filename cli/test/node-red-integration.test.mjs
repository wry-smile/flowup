...
test(
  'assembled resources, editor config, locales, and icons load in real Node-RED',
  {
  ...
  async t => {
    ...
    await createBuiltPackage({
      ...
      icons: {
        'node-a.svg': '<svg xmlns="http://www.w3.org/2000/svg"><title>node-a-icon</title></svg>\n',
      },
      locales: {
    ...
    try {
      ...
      if (error?.code === 'EPERM') {
        if (process.env.FLOWUP_REQUIRE_NODE_RED_INTEGRATION) throw error
    ...
    assert.match(editorConfig, /\/resources\/@flowup-test\/assembled\/node-a\/shared\.txt/)
    assert.match(editorConfig, /resources\/@flowup-test\/assembled\/node-a\/shared\.txt/)
    assert.equal(
    ...
    const messages = await fetchJson(`${baseUrl}/nodes/${assembleName}/node-a/messages?lng=en-US`)
