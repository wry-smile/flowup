...
- Each generated UnoCSS client imports `virtual:uno.css`. In generated multi-entry packages, `gen add` updates `flowup.config.ts` after every new entry. Flowup deduplicates the child imports during grouped builds. Mount each app in an element with `data-flowup-scope="<package-name>"`. The attribute value must equal the config scope.
