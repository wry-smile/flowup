---
name: flowup-unocss
description: Configure scoped UnoCSS in Flowup Vue and Svelte Node-RED editors, including generated templates, overlay scopes, and CSS isolation boundaries.
metadata:
...
- For a manually configured client, add `unocss` as a development dependency. Add `UnoCSS({ presets: [presetFlowupWind4({ scope: '<package-name>' })] })` from `unocss/vite` and `@wry-smile/flowup` to the editor Vite plugins. Put UnoCSS before the Svelte plugin.
